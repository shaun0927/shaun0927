...
2. Claude AI가 스크린샷을 분석하여 대회 정보 추출
...
### 3. Anthropic API 키 설정 (중요!)

Claude AI API를 사용하려면 API 키가 필요합니다.
...
**주의**: `.env` 파일은 절대 GitHub에 업로드하지 마세요! `.gitignore`에 추가되어 있습니다.
...
- **`one_click_update.py`**: 전체 프로세스 자동화 마스터 스크립트
- **`auto_update.py`**: Selenium으로 Dacon/Kaggle 페이지 스크린샷 캡처
- **`analyze_screenshots.py`**: Claude AI로 스크린샷 분석 및 데이터 추출
...
└──────────┬──────────┘
           ...
           ▼
┌─────────────────────┐
│ 2. AI 이미지 분석   │ ← analyze_screenshots.py
│   - Claude Vision   │
│   - 대회 정보 추출  │
└──────────┬──────────┘
           │
...
**해결책**: 위의 "Anthropic API 키 설정" 섹션 참조
...
이제 `UPDATE.bat`를 더블클릭하는 것만으로 GitHub 프로필이 자동으로 업데이트됩니다!
